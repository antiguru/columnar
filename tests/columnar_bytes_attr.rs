//! Tests for the `#[columnar(bytes)]` field attribute, which stores a `Vec<u8>` field through
//! `Strings` (native `&[u8]` reference) while keeping the field's `Vec<u8>` type.
...
#[derive(Columnar, Clone, Debug, PartialEq)]
struct TupleRecord(u64, #[columnar(bytes)] Vec<u8>);
